...
+function ($, window, document) {
    ...
    Plugin.prototype = {
        _init: function () {
            ...
            if (groupLength == 0) {
                console.error('Missing icons config!');
                return false;
            }

            var emoji_container = '<div class="emoji_container" id="emoji_container_' + that.index + '">';
            ...
            for (var i = 0; i < groupLength; i++) {
                name = iconsGroup[i].name;
                path = iconsGroup[i].path;
                maxNum = iconsGroup[i].maxNum;
                excludeNums = iconsGroup[i].excludeNums;
                file = iconsGroup[i].file || '.jpg';
                prefix = iconsGroup[i].prefix;
                title = iconsGroup[i].title;
                index = 0;
                if (!name || !path || !maxNum || !prefix) {
                    console.error('The ' + i + ' index of icon groups has error config!');
                    continue;
                }
                panel = '<div id="emoji_' + prefix + '" class="emoji_icons" style="' + (i == 0 ? '' : 'display:none;') + '"><ul>';
                for (var j = 1; j <= maxNum; j++) {
                    if (excludeNums && excludeNums.indexOf(j) >= 0) {
                        continue;
                    }
                    panel += '<li><a data-emoji_code="' + flag + prefix + '_' + j + flag + '" data-index="' + index + '" title="' + (title && title[j] ? title[j] : '') + '"><img src="' + path + j + file + '"/></a></li>';
                    index++;
                }
                panel += '</ul></div>';
                emoji_content += panel;
                emoji_tab += '<li data-emoji_tab="emoji_' + prefix + '" class="' + (i == 0 ? 'selected' : '') + '" title="' + name + '">' + name + '</li>';
            }
            emoji_content += '</div>';
            ...
            $('#emoji_container_' + that.index).css({'top': btnTop + 'px', 'left': btnLeft + 'px'});

            $('#emoji_container_' + that.index + ' .emoji_content').mCustomScrollbar({
                theme: 'minimal-dark',
            ...
            $(document).on({
                'click': function (e) {
                    ...
                    var code,
                        ...
                        insertHtml;
                    if (target === $(btn)[0]) {
                        $('#emoji_container_' + that.index)[that.toggleFunc]();
                        that.$content.focus();
                    } else if ($(target).parents('#emoji_container_' + that.index).length > 0) {
                        code = $(target).data('emoji_code') || $(target).parent().data('emoji_code');
                        tab = $(target).data('emoji_tab');
                        if (code) {
                            if (field.nodeName === 'DIV') {
                                imgSrc = $('[data-emoji_code="' + code + '"] img').attr('src');
                            ...
                            if (!$(target).hasClass('selected')) {
                                $('#emoji_container_' + that.index + ' .emoji_icons').hide();
                                $('#emoji_container_' + that.index + ' #' + tab).show();
                                $(target).addClass('selected').siblings().removeClass('selected');
                            }
                        } else if ($(target).hasClass('emoji_tab_prev')) {
                            if (pageIndex > 1) {
                                $('#emoji_container_' + that.index + ' .emoji_tab_list ul').css('margin-left', ('-503' * (pageIndex - 2)) + 'px');
                                pageIndex--;
                            }

                        } else if ($(target).hasClass('emoji_tab_next')) {
                            if (pageIndex < pageCount) {
                                $('#emoji_container_' + that.index + ' .emoji_tab_list ul').css('margin-left', ('-503' * pageIndex) + 'px');
                                pageIndex++;
                            }
                        }
                        that.$content.focus();
                    } else if ($('#emoji_container_' + that.index + ':visible').length > 0) {
                        that.hide();
                        that.$content.focus();
                    }
                }
            ...
            $('#emoji_container_' + that.index + ' .emoji_icons a').mouseenter(function () {
                var index = $(this).data('index');
                if (parseInt(index / 5) % 2 == 0) {
                    $('#emoji_container_' + that.index + ' .emoji_preview').css({"left": "auto", "right": 0});
                } else {
                    $('#emoji_container_' + that.index + ' .emoji_preview').css({"left": 0, "right": "auto"});
                }
                var src = $(this).find('img').attr('src');
                $('#emoji_container_' + that.index + ' .emoji_preview img').attr('src', src);
                $('#emoji_container_' + that.index + ' .emoji_preview').show();
            });

            $('#emoji_container_' + that.index + ' .emoji_icons a').mouseleave(function () {
                $('#emoji_container_' + that.index + ' .emoji_preview img').removeAttr('src');
                $('#emoji_container_' + that.index + ' .emoji_preview').hide();
            });
        },
        ...
        _init: function () {
            ...
            if (groupLength > 0) {
                for (var i = 0; i < groupLength; i++) {
                    ...
                    var pattern = flag + prefix + '_(\\d+)' + flag;
                    that.$content.html(that.$content.html().replace(new RegExp(pattern, 'gm'), '<img src="' + path + '$1' + file + '"/>'));
                }
            }
        }
    };
